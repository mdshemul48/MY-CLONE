...
from info import backend_url_link
...
class Db_request_api:
    ...
    def log_error(self, bot_title: str, error_text: str):
        ...
        response = self.api.put(backend_url_link + "/error", json=error)
    ...
    def update_content(self, title: str, content: dict):
        ...
        response = self.api.patch(backend_url_link + "/movies/edit", json=data)
    ...
    def bot_status(self, update: dict):
        response = self.api.post(backend_url_link + "/bot-status", json=update)
        if not response.ok:
